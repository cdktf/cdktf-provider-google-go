...
func (t *jsiiProxy_TranscoderJobConfigManifestsList) validateResolveParameters(_context cdktf.IResolveContext) error {
	if _context == nil {
...
func (j *jsiiProxy_TranscoderJobConfigManifestsList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*TranscoderJobConfigManifests:
		val := val.(*[]*TranscoderJobConfigManifests)
	...
	case []*TranscoderJobConfigManifests:
		val_ := val.([]*TranscoderJobConfigManifests)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*TranscoderJobConfigManifests; received %#v (a %T)", val, val)
		}
	}
