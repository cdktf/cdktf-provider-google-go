...
func (m *jsiiProxy_MonitoringMetricDescriptorLabelsList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
...
func (j *jsiiProxy_MonitoringMetricDescriptorLabelsList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*MonitoringMetricDescriptorLabels:
		val := val.(*[]*MonitoringMetricDescriptorLabels)
	...
	case []*MonitoringMetricDescriptorLabels:
		val_ := val.([]*MonitoringMetricDescriptorLabels)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*MonitoringMetricDescriptorLabels; received %#v (a %T)", val, val)
		}
	}
