...
func (j *jsiiProxy_ComputeInstanceFromTemplateScratchDiskList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*ComputeInstanceFromTemplateScratchDisk:
		val := val.(*[]*ComputeInstanceFromTemplateScratchDisk)
	...
	case []*ComputeInstanceFromTemplateScratchDisk:
		val_ := val.([]*ComputeInstanceFromTemplateScratchDisk)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*ComputeInstanceFromTemplateScratchDisk; received %#v (a %T)", val, val)
		}
	}
